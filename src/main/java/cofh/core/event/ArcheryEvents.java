...
public class ArcheryEvents {
    ...
    public static void handleItemUseTickEvent(LivingEntityUseItemEvent.Tick event) {
        ...
        int encQuickDraw = getItemEnchantmentLevel(getEnchantment(ModIds.ID_ENSORCELLATION, ID_QUICK_DRAW), event.getItem());
    ...
    public static void handleLivingHurtEvent(LivingHurtEvent event) {
        ...
        if (source.msgId.equals(DAMAGE_ARROW)) {
            int encVolley = getHeldEnchantmentLevel((LivingEntity) attacker, getEnchantment(ModIds.ID_ENSORCELLATION, ID_VOLLEY));
            if (encVolley > 0) {
